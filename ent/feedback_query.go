...
func (fq *FeedbackQuery) QueryResponses() *PromptResponseQuery {
	query := (&PromptResponseClient{config: fq.config}).Query()
...
func (fq *FeedbackQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (fq *FeedbackQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (fq *FeedbackQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (fq *FeedbackQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (fq *FeedbackQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (fq *FeedbackQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := fq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (fq *FeedbackQuery) WithResponses(opts ...func(*PromptResponseQuery)) *FeedbackQuery {
	query := (&PromptResponseClient{config: fq.config}).Query()
...
func (fq *FeedbackQuery) loadResponses(ctx context.Context, query *PromptResponseQuery, nodes []*Feedback, init func(*Feedback), assign func(*Feedback, *PromptResponse)) error {
	...
	query.Where(predicate.PromptResponse(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(feedback.ResponsesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.feedback_responses
		if fk == nil {
			return fmt.Errorf(`foreign-key "feedback_responses" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (fq *FeedbackQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(feedback.Table, feedback.Columns, sqlgraph.NewFieldSpec(feedback.FieldID, field.TypeUUID))
	_spec.From = fq.sql
