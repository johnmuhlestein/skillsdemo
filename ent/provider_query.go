...
func (pq *ProviderQuery) QueryAppointments() *AppointmentQuery {
...
func (pq *ProviderQuery) WithAppointments(opts ...func(*AppointmentQuery)) *ProviderQuery {
	query := (&AppointmentClient{config: pq.config}).Query()
...
func (pq *ProviderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Provider, error) {
	...
	if query := pq.withAppointments; query != nil {
		if err := pq.loadAppointments(ctx, query, nodes,
			func(n *Provider) { n.Edges.Appointments = []*Appointment{} },
			func(n *Provider, e *Appointment) { n.Edges.Appointments = append(n.Edges.Appointments, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProviderQuery) loadAppointments(ctx context.Context, query *AppointmentQuery, nodes []*Provider, init func(*Provider), assign func(*Provider, *Appointment)) error {
	...
	query.Where(predicate.Appointment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(provider.AppointmentsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.provider_appointments
		if fk == nil {
			return fmt.Errorf(`foreign-key "provider_appointments" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "provider_appointments" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
