...
func (sq *SurveyQuery) QueryPrompts() *PromptQuery {
	query := (&PromptClient{config: sq.config}).Query()
...
func (sq *SurveyQuery) QueryAppointments() *AppointmentQuery {
...
func (sq *SurveyQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (sq *SurveyQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (sq *SurveyQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (sq *SurveyQuery) WithPrompts(opts ...func(*PromptQuery)) *SurveyQuery {
	query := (&PromptClient{config: sq.config}).Query()
...
func (sq *SurveyQuery) WithAppointments(opts ...func(*AppointmentQuery)) *SurveyQuery {
	query := (&AppointmentClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (sq *SurveyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Survey, error) {
	...
	if query := sq.withPrompts; query != nil {
	...
	if query := sq.withAppointments; query != nil {
		if err := sq.loadAppointments(ctx, query, nodes,
			func(n *Survey) { n.Edges.Appointments = []*Appointment{} },
			func(n *Survey, e *Appointment) { n.Edges.Appointments = append(n.Edges.Appointments, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SurveyQuery) loadPrompts(ctx context.Context, query *PromptQuery, nodes []*Survey, init func(*Survey), assign func(*Survey, *Prompt)) error {
	...
	query.Where(predicate.Prompt(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(survey.PromptsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.survey_prompts
		if fk == nil {
			return fmt.Errorf(`foreign-key "survey_prompts" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (sq *SurveyQuery) loadAppointments(ctx context.Context, query *AppointmentQuery, nodes []*Survey, init func(*Survey), assign func(*Survey, *Appointment)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Survey)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Appointment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(survey.AppointmentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.survey_appointments
		if fk == nil {
			return fmt.Errorf(`foreign-key "survey_appointments" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "survey_appointments" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (sq *SurveyQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(survey.Table, survey.Columns, sqlgraph.NewFieldSpec(survey.FieldID, field.TypeUUID))
	_spec.From = sq.sql
